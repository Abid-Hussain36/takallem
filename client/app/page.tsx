...
export default function Home() {
  ...
  const refModules = useRef<Set<number> | null>(userCourseProgress?.ref_modules ? new Set(userCourseProgress.ref_modules) : null);
  ...
  useEffect(() => {
    const getModules = async () => {
      ...
      // If no auth token OR no user, redirect to login (but don't modify state to avoid loop)
      if(!authToken || !user){
        if(authToken){
          localStorage.removeItem("token");
        }
