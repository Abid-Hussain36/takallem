...
from app.utils.constants import PRONOUNCIATION_BASE_URL, MSA, build_check_pronounciation_messages
...
class PronounciationService():
    async def check_pronounciation(
        ...
        try:
            ...
            chat_response_json = json.loads(chat_response_content)
            pronounciation_check_response = LetterPronounciationResponse(
                status=status,
                feedback=chat_response_json["feedback"],
                mistake_tags=chat_response_json["mistake_tags"],
                performance_reflection=chat_response_json["performance_reflection"],
            )
            print("Parsed chat output")
            print(pronounciation_check_response)
        except Exception:
            print("Error parsing chat response")
    ...
    async def check_pronounciation_test(
        self,
        user_audio: UploadFile,
        word: str
    ):
